...
(defun configuration-layer/load ()
  ...
  (let ((changed-since-last-dump-p nil))
                    ...
                    (let* ((obj (configuration-layer/get-layer layer))
                      ...
                      (dolist (pkg pkgs)
                        (let ((pkg-name (if (listp pkg) (car pkg) pkg)))
                          (cl-pushnew pkg-name all-other-packages 'test :equal))))))
                (configuration-layer//filter-distant-packages
  ...
  (let* ((layer-name (if (listp layer-specs) (car layer-specs) layer-specs))
    ...
    (if (or (null dir)
      ...
      (let* ((dir (file-name-as-directory dir))
        ...
        (oset obj dir dir)
        (when usedp
          (oset obj disabled disabled)
          (oset obj enabled enabled)
    ...
    (let (result)
      (dolist (pkg-name pkg-names)
        ...
        (let* ((deps
          ...
          (when install-deps
            (setq result (append result install-deps))))
        (when (funcall filter pkg-name)
          (push pkg-name result)))
...
LAYER must not be the owner of PKG."
  (let* ((owner (configuration-layer/get-layer (car (oref pkg owners))))
         (disabled (when owner (oref owner disabled)))
         (enabled (when owner (oref owner enabled))))
    (and owner
         (not (memq nil (mapcar
                         (lambda (dep-pkg)
                           (let ((pkg-obj (configuration-layer/get-package dep-pkg)))
      ...
      (let* ((pkg-sym (car pkg))
        ...
        (dolist (dep deps)
          (let* ((dep-sym (car dep))
            ...
            (puthash dep-sym
                     (if value (cl-pushnew pkg-sym value :test 'equal) (list pkg-sym))
                     result)))))
    result))
    ...
    (dolist (pkg package-alist)
      (let ((pkg-sym (car pkg)))
        (unless (memq pkg-sym packages)
          (cl-pushnew pkg-sym imp-pkgs :test 'equal))))
    imp-pkgs))
  ...
  (let (result)
    (dolist (imp-pkg implicit-pkgs)
      (when (configuration-layer//is-package-orphan
        ...
        (cl-pushnew imp-pkg result :test 'equal)))
    result))
      ...
      (when (assq pkg-sym package-archive-contents)
        (let* ((deps (mapcar 'car
          ...
          (dolist (pkg (cons pkg-sym elpa-deps))
            ;; avoid duplicates
            (cl-pushnew pkg result :test 'equal)))))
    result))
